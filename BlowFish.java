	...
	// Substitution boxes each string is a 32 bit hexadecimal value.
		...
		public static void main(String args[]) {
			// storing 2^32 in modVal
				...
				// to create connection and database collection and insert data into database 
				// Adding this ArrayList to Mongodb database
				MongoClient mongo = new MongoClient("localhost", 27017); 
		        DB db = mongo.getDB("Blowfishmongo");
		        DBCollection collectionEmp = db.getCollection("Employee");
		        insertAllDocuments(collectionEmp,alldata);

		        // Applying BlowFish Encryption on Employee collection
				String cipherText;
				String key = "aabb09182736ccdd";
				DBCollection collectionEmpenc = db.getCollection("EncEmployee");
				keyGenerate(key);
				System.out.println("**********Encryption*************");
				for(Employee emp:alldata) {
			        String encobj = emp.toString().replace(" ","").replace("-","");
			        //.replace("-","A")
			        //.replace("-","Z").replace(".","Z")
			        System.out.println(encobj);
			        
				    cipherText = encrypt(encobj);
			...
			System.out.println("final plain is " + plain);
			*/
...
arr[0]= new Employee(120274,"battu","Front-End",1000);
arr[1]= new Employee(120285,"boni","Back-End",2000);
arr[2] = new Employee(120271,"krishna","Devops",3000);
// to create connection and database collection and insert data into database            
MongoClient mongoclient = new MongoClient("localhost",27017);
MongoDatabase db = mongoclient.getDatabase("javamongo");
MongoCollection<Document> table = db.getCollection("Employee");

String original;
String key = "aabb09182736ccdd";
MongoCollection<Document> enc = db.getCollection("EmployeeBF");
for(int i=0;i<n;i++){
    Document doc = new Document("eid",arr[i].getEmployeeId());
    doc.append("ename",arr[i].getEmployeeName());
    doc.append("edept",arr[i].getEmployeeDesignation());
    doc.append("salary",arr[i].getSalary());
    table.insertOne(doc);
    
    // here original is palin text
    original= doc.toString();
    if(original instanceof String){
       System.out.println("it is string"+ original);
       System.out.println(key);
    }
    
    // we need to convert the document to string format for encryption process
    
    // we will generate the keys for rounds 18 keys
    keyGenerate(key);
    System.out.println("**********Encryption*************");
    String cipherText = encrypt(original);
    System.out.println("Cipher Text: " + cipherText);
    
    // we will add these cipher text to EmployeeBF collection
    Document encdoc = new Document("cipher",cipherText);
    encdoc.append("eid",arr[i].getEmployeeId());
    enc.insertOne(encdoc);
}
*/	